...
class BsFormHtmlBuilder {
	...
	const BS_VALIDATED_INVALID_CLASS = 'is-invalid';
	const BS_VALIDATED_VALID_CLASS = 'is-valid';
...
// 		$view->getHtmlBuilder()->meta()->addLibrary(new BootstrapLibrary());
	...
	public function getInputFileWithLabelGroup($propertyExpression = null, BsComposer $bsComposer = null, $label = null,
		...
		$propertyPath = $this->createPropertyPath($propertyExpression);
		$bsConfig = $this->createBsConfig($bsComposer);
		$controlAttrs = $this->createFormControlAttrs($propertyPath, $bsConfig, null, 'form-control');
...
//		if ($bsComposer !== null) {
		...
		$selectControlAttrs = $this->createFormSelectAttrs($propertyPath, $bsConfig);
//		}
		...
		// @dev: tomekk -> i think we do not need this, just the array that should be passed to new BsUiOutfitter()
		if (empty($selectControlAttrs)) {
			$selectControlAttrs = ['class' => 'form-select'];
		}

		$formHtmlMeta = $this->formHtml->meta();
		if ($formHtmlMeta->isDispatched() && $formHtmlMeta->hasErrors()) {
			if ($formHtmlMeta->hasErrors($propertyPath)) {
				$controlAttrs = HtmlUtils::mergeAttrs($controlAttrs, ['class' => self::BS_VALIDATED_INVALID_CLASS]);
				$checkControlAttrs = HtmlUtils::mergeAttrs($checkControlAttrs, ['class' => self::BS_VALIDATED_INVALID_CLASS]);
				$selectControlAttrs = HtmlUtils::mergeAttrs($selectControlAttrs, ['class' => self::BS_VALIDATED_INVALID_CLASS]);
			} else {
				$controlAttrs = HtmlUtils::mergeAttrs($controlAttrs, ['class' => self::BS_VALIDATED_VALID_CLASS]);
				$checkControlAttrs = HtmlUtils::mergeAttrs($checkControlAttrs, ['class' => self::BS_VALIDATED_VALID_CLASS]);
				$selectControlAttrs = HtmlUtils::mergeAttrs($selectControlAttrs, ['class' => self::BS_VALIDATED_VALID_CLASS]);
			}
		}

		$bsUiOutfitter = new BsUiOutfitter($outfitConfig, $bsConfig, $controlAttrs, $checkControlAttrs, $selectControlAttrs);
